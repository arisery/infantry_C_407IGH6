...
#define SPI_user	hspi2

#define SPI_WAIT_TIMEOUT			((uint16_t)0xFF)


/** 配置和选项定义 */

#define DYNAMIC_PACKET      0		//1:动态数据包, 0:固定
#define FIXED_PACKET_LEN    18		//包长度
...
#define spi_set_nss_high( )			SPI_NSS_GPIO_Port->ODR |= SPI_NSS_Pin
#define spi_set_nss_low( )			SPI_NSS_GPIO_Port->ODR &= (uint32_t)( ~((uint32_t)SPI_NSS_Pin ))
