...
uint8_t NRF24L01_Read_Reg( uint8_t RegAddr )
{
    ...
    spi_transmit( NRF_READ_REG | RegAddr );	//读命令 地址
  spi_receive( &btmp );				//读数据
...
void NRF24L01_Write_Reg( uint8_t RegAddr, uint8_t Value )
{
    ...
    spi_transmit( NRF_WRITE_REG | RegAddr );	//写命令 地址
    spi_transmit( Value );			//写数据
...
void NRF24L01_Write_Buf( uint8_t RegAddr, uint8_t *pBuf, uint8_t len )
{
    ...
    spi_transmit( NRF_WRITE_REG | RegAddr );	//写命令 地址
    HAL_SPI_Transmit(&SPI_user,pBuf,len,0xff);
    /*
    for( i = 0; i < len; i ++ )
    {
    	spi_transmit( *( pBuf + i ) );		//写数据
    }
	*/
    RF24L01_SET_CS_HIGH( );		//取消片选
}
...
void NRF24L01_Flush_Rx_Fifo( void )
{
    ...
    spi_transmit( FLUSH_RX );	//清RX FIFO命令
...
void NRF24L01_Reuse_Tx_Payload( void )
{
    ...
    spi_transmit( REUSE_TX_PL );		//重新使用上一包命令
...
uint8_t NRF24L01_Read_Status_Register( void )
{
    ...
    spi_transmit( NRF_READ_REG + STATUS );	//读状态寄存器
    spi_receive(&Status);
    RF24L01_SET_CS_HIGH( );		//取消片选
...
uint8_t NRF24L01_Clear_IRQ_Flag( uint8_t IRQ_Source )
{
    ...
    RF24L01_SET_CS_LOW( );			//片选
    spi_transmit( NRF_WRITE_REG + STATUS );	//写状态寄存器命令
    spi_transmit( IRQ_Source | btmp );		//清相应中断标志
    RF24L01_SET_CS_HIGH( );			//取消片选
...
uint8_t NRF24L01_Read_Top_Fifo_Width( void )
{
    ...
    spi_transmit( R_RX_PL_WID);	//读FIFO中数据宽度命令
    spi_receive (&btmp);	//读数据
...
uint8_t NRF24L01_Read_Rx_Payload( uint8_t *pRxBuf )
{
    ...
    RF24L01_SET_CS_LOW( );		//片选
    spi_transmit(RD_RX_PLOAD);			//读有效数据命令
	/*
    for( PipeNum = 0; PipeNum < Width; PipeNum ++ )
    {
    	spi_receive( pRxBuf + PipeNum );		//读数据
    }
    */
    HAL_SPI_Receive(&SPI_user,pRxBuf,Width,0xfff);
    RF24L01_SET_CS_HIGH( );		//取消片选
...
void NRF24L01_Write_Tx_Payload_Ack( uint8_t *pTxBuf, uint8_t len )
{
    ...
    HAL_SPI_Transmit(&SPI_user, pTxBuf,len,0xff );
/*
...
void NRF24L01_Write_Tx_Payload_NoAck( uint8_t *pTxBuf, uint8_t len )
{
    ...
    RF24L01_SET_CS_LOW( );	//片选
    spi_transmit(WR_TX_PLOAD_NACK);	//发送命令
    	HAL_SPI_Transmit(&SPI_user, pTxBuf,len,0xff );			//发送数据
    RF24L01_SET_CS_HIGH( );		//取消片选
}
...
void init_rx()
{
    ...
    NRF24L01_Write_Reg( DYNPD, ( 1 << 0 ) ); 	//使能通道1动�?�数据长�????
    NRF24L01_Write_Reg( FEATRUE, 0x07 );
    NRF24L01_Read_Reg( DYNPD );
    NRF24L01_Read_Reg( FEATRUE );
    ...
    NRF24L01_Write_Reg( CONFIG, /*( 1<<MASK_RX_DR ) |*/		//接收中断
     ...
     HAL_Delay(10);
      NRF24L01_Write_Reg( EN_AA, ( 1 << ENAA_P0 ) );   		//通道0自动应答
      NRF24L01_Write_Reg( EN_RXADDR, ( 1 << ERX_P0 ) );		//通道0接收
      NRF24L01_Write_Reg( SETUP_AW, AW_5BYTES );     			//地址宽度 5个字�????
      NRF24L01_Write_Reg( SETUP_RETR, ARD_250US |
                          ( REPEAT_CNT & 0x0F ) );         	//重复等待时间 250us
      NRF24L01_Write_Reg( RF_CH, 50 );             			//初始化�?�道
      NRF24L01_Write_Reg( RF_SETUP, 0x0F );

      NRF24L01_Set_TxAddr( &addr[0], 5 );                      //设置TX地址
      NRF24L01_Set_RxAddr( 0, &addr[0], 5 );
      NRF24L01_Write_Reg( FEATRUE, 0x01 );
      RF24L01_SET_CE_HIGH( );
}
void init_tx()
{
    ...
    NRF24L01_Write_Reg( CONFIG,( 1<<MASK_TX_DS ) |		//屏蔽发送中断
                                        ( 1 << EN_CRC ) |     //使能CRC 1个字�????
                                        ( 1 << PWR_UP ) );    //�????启设�????
      HAL_Delay(10);
      NRF24L01_Write_Reg( EN_AA, ( 1 << ENAA_P0 ) );   		//通道0自动应答
...
uint8_t drv_spi_read_write_byte( uint8_t TxByte )
{
	...
	while( RESET == __HAL_SPI_GET_FLAG( &SPI_user, SPI_FLAG_TXE ) )		//等待发�?�缓冲区为空
	{
	...
	l_WaitTime = SPI_WAIT_TIMEOUT / 2;		//重新设置接收等待时间(因为SPI的�?�度很快，正常情况下在发送完成之后会立即收到数据，等待时间不�??要过�??)
	SPI2->DR = TxByte;	//发�?�数�??

	while( RESET == __HAL_SPI_GET_FLAG( &hspi1, SPI_FLAG_RXNE ))		//等待接收缓冲区非�??
	{
		if( SPI_WAIT_TIMEOUT == ++l_WaitTime )
		{
			break;			//如果等待超时则�??�??
...
{
//	HAL_GPIO_WritePin( SPI_NSS_GPIO_PORT, SPI_NSS_GPIO_PIN,RESET);			//拉低片�??
	while( Length-- )
	{
	...
	//HAL_GPIO_WritePin( SPI_NSS_GPIO_PORT, SPI_NSS_GPIO_PIN,SET);				//拉高片�??
}
void spi_transmit(uint8_t data)
{
	HAL_SPI_Transmit(&SPI_user,&data,1,0xff);
}
void spi_receive(uint8_t* data)
{
	HAL_SPI_Receive(&SPI_user,data,1,0xff);
}
void read_string(uint8_t reg,uint8_t *data,uint8_t length)
{
	HAL_SPI_Transmit(&SPI_user,&reg,1,0xff);
	HAL_SPI_Receive(&SPI_user,data,length,0xff);
}
