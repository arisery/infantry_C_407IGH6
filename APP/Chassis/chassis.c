...
void chassis_mode_set(chassis_struct_t* chassis_t)
{
	...
	{
		chassis_t->move_mode=independent;

		}
	else if(switch_is_mid(chassis_t->RC->rc.s[0]))
		{
			chassis_t->move_mode=sync;

			}
	else if(switch_is_up(chassis_t->RC->rc.s[0]))
		{
			chassis_t->move_mode=contrary;
...
void chassis_data_update(chassis_struct_t* chassis_update)
{
	...
	chassis_update->vx = (-chassis_update->wheel[0].speed
			+ chassis_update->wheel[1].speed
			+ chassis_update->wheel[2].speed
			- chassis_update->wheel[3].speed)
...
{
	float vx_set = 0.0f, vy_set = 0.0f,wz_set=0.0f, angle_set = 0.0f,wz_channel=0.0f;
...
if(switch_is_down(chassis_control->RC->rc.s[1]))
{
...
if(chassis_control->move_mode==independent)
{

}
else if(chassis_control->move_mode==sync)
{
	//发生相同信号
}
...
//遥控器原始通道值
	...
	rc_deadline_limit(
			chassis_move_rc_to_vector->RC->rc.ch[CHASSIS_X_CHANNEL],
			vx_channel, RC_DEADLINE);
	rc_deadline_limit(
			chassis_move_rc_to_vector->RC->rc.ch[CHASSIS_Y_CHANNEL],
...
uint8_t i;
	...
	for (i = 0; i < 4; i++)
		{
		chassis_pid->wheel[i].set_current=PID_Calc(&chassis_pid->motor_speed_pid[i],
					chassis_pid->wheel[i].speed,
 ...
 *
 */
void chassis_mecanum_wheel_speed(const float vx_set,
		const float vy_set, const float wz_set, float wheel_speed[4])
{
...
void chassis_task()
{
	...
	{
		chassis.wheel[0].set_current=
					chassis.wheel[1].set_current=
					chassis.wheel[2].set_current=
					chassis.wheel[3].set_current=0;
	}
	set_motor_voltage_CAN1(chassis.wheel[0].set_current,
			chassis.wheel[1].set_current,
			chassis.wheel[2].set_current,
